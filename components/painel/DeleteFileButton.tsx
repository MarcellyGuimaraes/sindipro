...
/**
 * Exclui um arquivo: remove o objeto do Storage (bucket downloads) e a linha
 * da tabela `files`. Confirma antes de apagar.
 */
export function DeleteFileButton({
  ...
  async function handleDelete() {
    ...
    const rm = await supabase.storage.from("downloads").remove([storagePath]);
    if (rm.error) {
    ...
    // 2) Remove os metadados da tabela.
