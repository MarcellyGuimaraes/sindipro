...
/**
 * Exclui um membro da diretoria: remove a foto do Storage (board-photos),
 * quando houver, e a linha da tabela board_members. Confirma antes.
 */
export function DeleteBoardMemberButton({
  id,
  name,
  storagePath,
}: {
  id: string;
  name: string;
  /** Caminho da foto no bucket (derivado da URL), se houver. */
  ...
  async function handleDelete() {
    ...
    if (storagePath) {
      await supabase.storage.from("board-photos").remove([storagePath]);
    }
